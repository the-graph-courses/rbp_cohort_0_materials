...
142 rows and 11 columns. Remember to use the `here()` function to allow
...
**Warning!**: while verifying your types, you may also need to handle your missing values (encoded as NAs or otherwise). It is up to you to handle those throughout the different tasks in this notebook.
...
Why do we perform this conversion? The [WHO (known as OMS in French) recommendations](https://apps.who.int/iris/bitstream/handle/10665/337001/9789240014886-eng.pdf) are in minutes per week, so we want to align with these measures.
...
Using esquisse and the subset you just made above, plot LPA distribution (minutes/week)
for male respondents, as a histogram.
...
1.  To make this subset, you will only keep the adults in the dataset
...
distributions (minutes/week) by adult age groups, as boxplots.
